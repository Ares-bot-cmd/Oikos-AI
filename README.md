...
FastAPI + SQLAlchemy + SQLite backend for the Oikos AI real-estate sales
...
**The frontend is included** (`templates/index.html` + `templates/style.css`
+ `templates/script.js`) and is wired to call this API with `fetch()`
instead of using hardcoded data — the whole app (backend + frontend)
runs from a single process. It also includes a live AI chat widget
(bottom-right) backed by the Claude API.
...
The chatbot needs an `ANTHROPIC_API_KEY` (get one at
https://console.anthropic.com/settings/keys). Without it, every other
tab still works fully — only the chat widget shows a "not configured"
...
On first startup, `oikos_ai.db` (SQLite file) is created automatically
...
Seeding only runs once; delete `oikos_ai.db` to reset and reseed.
...
| POST   | `/api/chat`                       | AI chatbot widget (needs `ANTHROPIC_API_KEY`) |

## 4. Frontend ↔ backend wiring

`templates/index.html` is served directly by FastAPI (mounted at `/`) and
...
Whichever host you use, set `ANTHROPIC_API_KEY` as an environment
variable in that host's dashboard (never commit it) or the chat widget
will respond with a "not configured" message instead of crashing.
...
├── templates/
│   ├── index.html        # Frontend markup — fetches live data from /api/*
│   ├── style.css          # Frontend styling
│   └── script.js           # Frontend logic incl. the chat widget
├── requirements.txt
├── .env.example
└── README.md
```
