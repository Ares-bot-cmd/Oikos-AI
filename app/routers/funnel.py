...
def fmt_inr(value: float) -> str:
    """Format a rupee amount the way it's actually written in India:
    Lakh (1,00,000) and Crore (1,00,00,000), not million/billion."""
...
def get_funnel(db: Session = Depends(get_db)):
    ...
    steps = [
        schemas.FunnelStep(label="Ad spend", value=m.ad_spend, display=fmt_inr(m.ad_spend)),
        schemas.FunnelStep(
        ...
        schemas.FunnelStep(label="Revenue", value=m.revenue, display=fmt_inr(m.revenue)),
    ]
